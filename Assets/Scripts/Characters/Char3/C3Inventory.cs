...
public class C3Inventory : MonoBehaviour
{
    ...
    {
        slotAmount = 10;
        inventoryPanel = GameObject.Find("C3InvPanel");
        slotPanel = inventoryPanel.transform.FindChild("C3SlotPanel").gameObject;
        for (int i = 0; i < slotAmount; i++)
        {
            items.Add(new Item());
            slots.Add(Instantiate(inventorySlot));
            slots[i].GetComponent<C3Slot>().id = i;
