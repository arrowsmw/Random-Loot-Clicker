...
public class C2Inventory : MonoBehaviour {
    ...
    {
        slotAmount = 10;
        inventoryPanel = GameObject.Find("C2InvPanel");
        slotPanel = inventoryPanel.transform.FindChild("C2SlotPanel").gameObject;
        for(int i=0; i<slotAmount; i++)
        {
            items.Add(new Item());
            slots.Add(Instantiate(inventorySlot));
            slots[i].GetComponent<C2Slot>().id = i;
